...
void streamCallbackRobot1(const sensor_msgs::PointCloud2& cloudRos) {
    ...
    try {
        tfListener.waitForTransform(fixedFrame, cloudFrame, ros::Time::now(), ros::Duration(10.0));
        tfListener.lookupTransform(fixedFrame, cloudFrame, ros::Time::now()), transform);
